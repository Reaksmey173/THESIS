...
The third check used automated tools on the same version of the code, in the same period: one for the packages the system depends on, one for the container images, and one for the team's own code and any passwords left in it. I ran the scanners and the manual testing on the same version at the same time, on purpose. It means the two sets of findings can be compared directly, rather than across a version that had changed in between.
...
I used AI tooling to help write and investigate code, under my direction. What to examine, whether a finding was real, and what to fix or defer were my decisions. I checked what the tooling reported against the running system before accepting it, and several reports turned out to be wrong. The placement taught me that AI can accelerate investigation and implementation, but it does not replace verification. Security decisions remained my responsibility, and outputs were accepted only after they were checked against the code, the running system, and the hospital's operational requirements.

\subsection*{Stakeholder-Driven Security Design and Appreciative Inquiry}
\addcontentsline{toc}{subsection}{Stakeholder-Driven Security Design and Appreciative Inquiry}

The access rules in this thesis were not derived from security frameworks alone. They came from a weekly process the wider team already used at Intercare, following the company's I-C-A-R-E values, Innovation, Compassionate, Accountability, Responsive, and Empowered, through a structured cycle: Define, a relevant Value, Discover, Dream, Design, and Deliver.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{figs/Figure3-Appreciative-Inquiry-cycle.png}
  \caption{The Appreciative Inquiry cycle used in weekly team meetings at Intercare, centred on the company's I-C-A-R-E values. Define and Deliver sit opposite each other in the cycle; Discover, Dream, and Design run between them, with the relevant Value checked at each stage.}
  \label{fig:appreciative-inquiry}
\end{figure}

The cycle repeated every Tuesday morning, and each week's Deliver step fed directly into the next week's Define.

Each Tuesday morning, the three-person team, covering data and adaptation, development and communication, and security, took turns presenting what we had worked on that week, with my supervisor present at every session, and often joined and supported by others from the IT team, the Finance team, the Coordinator team, or the Doctors. For each topic, the person presenting first defined it, then named which of the five values it connected to, before discovering the detail of what had been done, what problem or question had come up, and what remained open. From there, the group discussed what they wanted next, their dream for that piece of work, before moving to how it would actually be designed, and finally who would deliver it.

This process was where the access rules in Chapter \ref{chap:impl} were actually decided, not just documented afterwards. Rather than designing permissions from a security framework downward, the team went through each role in the workflow in turn, the surgeon, the anaesthetist, Finance, and the Coordinator, and discussed how each one actually worked on paper before agreeing how the same responsibility should work in the system. The clearest example of this is the shift described in Section \ref{sec:ps-background}: on paper, the Coordinator was a required step between the doctors and Finance. Deciding whether that should carry over into the digital system, or whether the doctors should pass a case to Finance directly, took several weeks of this weekly cycle to settle, since it meant Finance absorbing a step that used to belong to someone else, and every actor needed to understand and agree with the resulting shape of the workflow before it was built. The rule that resulted, described in Section 5.1.2, exists because of that discussion, not because of any security standard.
...
\hline
HL7 FHIR & R5 (guidance cited) & Security section only \\
\hline
...
\hline
STRIDE & N/A & All six categories \\
\hline
CIS Docker Benchmark & N/A & Not applied; planned for a later phase \\
\hline
OWASP Testing Guide & N/A & Not applied; planned for a later phase \\
\hline
\end{tabular}
\end{table}

Each source did one job. The NIST Cybersecurity Framework gave the work its structure: the seven phases in Figure \ref{fig:seven-phases} follow its six functions \cite{nistcsf2}. The system's data model already follows FHIR, so FHIR's security section was the obvious place to look for decisions about signing in and encryption \cite{hl7fhir}. Citing FHIR's current security guidance is a reference to the most recent published guidance available, not a claim that the deployed FHIR server itself implements that same specification release; Table \ref{tab:tools} in Section \ref{sec:met-tools} gives the actual server and version in use. NIST SP 800-162 gave the access control model \cite{nist80162}. The OWASP Top 10 was the list I checked the code against \cite{owasp2021}. The National Bank of Cambodia's guidelines gave something to measure the result against, rather than a rule to comply with \cite{nbc2026}. The OWASP ASVS gave a second, more granular checklist: a separate pass at Level 2 across all fourteen chapters of the 4.0.3 edition, reported alongside the Top 10 results in Chapter \ref{chap:result} \cite{owaspasvs}. STRIDE gave the six categories used in the threat model described next \cite{stride}.
...
I used STRIDE: spoofing, tampering, repudiation, information disclosure, denial of service, and elevation of privilege, to sort the threats this system faces. This was a retrospective threat analysis: I applied it after the system was built, not before. Its job was to file what testing had already found, and to check that no kind of threat had been missed. STRIDE gave the six categories. My findings filled them.

STRIDE suited this project for two practical reasons. The system was small enough that every part could be tested directly, rather than estimated in advance, so no separate scoring step was needed before testing began. The work was also done by one person, not a team, so a method requiring coordination across multiple stakeholders would not have fit.
...
\caption{STRIDE categories and how each was addressed.}
\label{tab:stride} \\
\hline
\textbf{Category} & \textbf{Threat in this system} & \textbf{Status} \\
\hline
\endfirsthead
...
\hline
\endlastfoot
Spoofing & Someone acts as a user they are not & Fixed; findings detailed in Section \ref{sec:result-manual} \\
\hline
Tampering & Case data changed by someone who should not be able to, or at a point when it should be locked & Mostly fixed; one open item remains (Section \ref{sec:conc-future}) \\
\hline
Repudiation & Someone denies an action, and the record cannot prove otherwise & Mostly fixed; one open item remains (Section \ref{sec:conc-future}) \\
\hline
Information disclosure & Data reaches someone who should not see it & Mostly fixed; a field-exposure gap and two unencrypted internal connections remain open (Section \ref{sec:conc-future}) \\
\hline
Denial of service & The system is knocked over so nobody can use it & Not addressed in this phase \\
\hline
Elevation of privilege & A user gains rights they were not granted & Fixed; findings detailed in Section \ref{sec:result-manual} \\
\hline
\end{longtable}
...
Denial of service was not addressed in this phase. Being reachable only inside the hospital lowers the risk from an outside attacker. It does not remove the risk from a compromised or careless account inside the hospital. Testing for it was not reached in the time available. It is understood to be covered by Canadia Bank (IT security department)'s separate review, though those findings are confidential and not reproduced in this thesis. It is recorded in Section \ref{sec:conc-future}.
...
\begin{table}[H]
\centering
\caption{Tools used in the system and in its verification, current versions at the time of writing.}
\label{tab:tools}
...
\hline
Database access and migrations & Prisma & 7.8.0 (client), 7.10.0 (CLI) \\
\hline
FHIR server & HAPI FHIR & 7.4.0 \\
\hline
Hospital information system & ANZER, read-only & N/A \\
\hline
Deployment & Docker / Docker Compose & 29.7.2 / v5.4.0 \\
\hline
Static analysis & Semgrep & N/A \\
\hline
Container image scanning & Trivy & N/A \\
\hline
