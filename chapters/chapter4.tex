...
Each source did one job. The NIST Cybersecurity Framework gave the work its structure, the seven phases in Figure \ref{fig:seven-phases} follow its six functions \cite{nistcsf2}. The system's data model already follows FHIR, so FHIR's security section was the obvious place to look for decisions about signing in and encryption \cite{hl7fhir}. NIST SP 800-162 gave the access control model \cite{nist80162}. The OWASP Top 10 was the list I checked the code against \cite{owasp2021}. The National Bank of Cambodia's guidelines gave something to measure the result against, rather than a rule to comply with \cite{nbc2026}. STRIDE gave the six categories used in the threat model described next \cite{stride}.
...
\begin{longtable}{p{2.3cm}p{4.1cm}p{7cm}}
...
Denial of service was not covered in this phase. The system is only reachable inside the hospital, so nobody outside can attack it, which made it the lowest priority of the six categories. It stayed in scope but was not reached: availability depends mostly on the server rather than the software, and the time ran out before I got to it. It is recorded in Section \ref{sec:conc-future} as work that remains.
...
\label{tab:tools}
\begin{tabular}{p{4.3cm}p{4.8cm}p{3.5cm}}
\toprule
\textbf{Purpose} & \textbf{Tool} & \textbf{Version} \\
\midrule
Identity provider & Keycloak & 26.6.4 \\
...
Deployment & Docker / Docker Compose & 29.6.1 / v5.1.4 \\
