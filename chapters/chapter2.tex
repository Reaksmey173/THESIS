...
The new system removes the Coordinator as a required stop. The doctors now pass the case to Finance directly. The Coordinator watches progress and can still edit when needed, which is what happened on paper too. What did not carry over is the asking. Before making a change, the Coordinator contacts whoever owns that part, the surgeon, the anaesthetist, or Finance by phone, message or in person. That happens outside the system. So the log shows who made the change, but not that anyone agreed to it.
...
On paper, the Coordinator had to walk over and ask before changing a doctor's form. In software, nothing physical stops anyone. So each rule the paper enforced had to become one of two things. Some rules the code blocks: the surgeon and anaesthetist can no longer edit a case once it reaches Finance, and the system refuses the change. Others it cannot block, because the system has no way to know whether the Coordinator phoned the surgeon first, so it allows the edit and records who made it.
...
The system is only reachable inside the hospital, so the realistic risk is a member of staff doing something they should not, rather than an outside attacker. Two checks remain: a penetration test by the hospital's IT Security Division, and a review of the container settings against the Center for Internet Security (CIS) Docker Benchmark. Neither had been done when this was written. Going live also required a signed non-disclosure agreement, hospital approval, and privacy safeguards, all agreed at the start of the project.
